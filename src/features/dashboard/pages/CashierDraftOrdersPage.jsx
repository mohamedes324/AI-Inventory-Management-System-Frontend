/**
 * @page CashierDraftOrdersPage
 * @description Dedicated page showing all draft orders for the current cashier.
 * Accessible via "View All" from the Cashier Dashboard.
 * Uses server-side pagination with the Orders API.
 */
...
export default function CashierDraftOrdersPage() {
      ...
      <header className="shrink-0 bg-background-card border-b border-border-primary px-4 sm:px-8 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm animate-fadeIn">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-text-primary tracking-tight">
                {t("cashierPages.draftOrders.title", "My Draft Orders")}
              </h1>
              <p className="text-text-muted text-sm mt-0.5">
                {t(
                  "cashierPages.draftOrders.description",
                  "Continue editing your unfinished orders"
                )}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          {totalCount > 0 && !loading && (
            <span className="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-500 text-xs font-bold px-3 py-1.5 rounded-full">
      ...
      <main className="flex-1 overflow-auto px-4 sm:px-8 py-6">
        {!loading && orders.length === 0 ? (
          <div className="h-full flex items-center justify-center py-24 animate-fadeIn">
            <EmptyState
              icon={
                <FileEdit size={36} className="text-text-muted" />
              }
              message={t(
                "cashierPages.draftOrders.empty",
                "No draft orders found"
              )}
              description={t(
                "cashierPages.draftOrders.emptyDesc",
                "You don't have any unfinished orders."
              )}
              action={
                <Button
                  size="sm"
